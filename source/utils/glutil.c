...
void glCompileShader_soloader(GLuint shader) {
    ...
    if (!skip_next_compile) {
        ...
        if (compile_status == GL_TRUE) {
            const GLsizei bin_bufsize = 128 * 1024;
            void *bin = vglMalloc(bin_bufsize);
            GLsizei len = 0;
            vglGetShaderBinary(shader, bin_bufsize, &len, bin);
            // If vglGetShaderBinary filled the whole buffer, the binary was almost
            // certainly truncated (its real size is unknown/unbounded) -- a corrupt/
            // truncated .gxp is what later crashed vitaGL's unserialize_shader on load,
            // so skip caching rather than write something we know is likely bad.
            if (len > 0 && len < bin_bufsize) {
                file_save(next_shader_fname, bin, len);
            } else {
                l_error("Shader %u binary looks truncated (len=%d, bufsize=%d), "
                        "not caching %s", shader, len, bin_bufsize, next_shader_fname);
            }
            vglFree(bin);
        } else {
            char info_log[512];
            ...
            l_error("Shader %u failed to compile, skipping binary dump for %s: %s",
                    shader, next_shader_fname, info_len > 0 ? info_log : "(no info log)");
        }
#endif
    }
...
void load_shader(GLuint shader, const char * string, size_t length) {
    ...
    if (file_exists(gxp_path) && file_load(gxp_path, &buffer, &size) && size > 0) {
