...
### Bug #25 (triage 2026-09-14, sobre corrida del 2026-09-12): mismo crash de Bug #23/#24 reaparece — NO es una regresión del fix de `translate_path`, es data faltante en la consola de este tester

- **Log/dump analizados:** `game_log_1789224796347.txt` + `psp2core-1789224818-0x00103a34cb-eboot.bin.psp2dmp`, corrida del 2026-09-12 10:53 -04 (la confirmación pedida al cierre del Bug #24, commit `2c27c27` del 2026-09-11 21:23 -04).
- **`psvita-toolkit analyze` sobre el dump:** Data abort idéntico a Bug #23/#24 — `PC = 0x981b40fc` (`_Locale_long_d_fmt+0xc`), `LR = 0x981b40e8` (`_Locale_d_fmt+0x10`), `R7 = 0`. Mismo mecanismo: `Lib::Open()` falla, `m_chunkCount = 0`, `SpriteMgr::LoadSprite()` no chequea el resultado y termina desreferenciando estado inválido.
- **¿El fix del Bug #24 (fallback a `GloftSGHP/`) está en este build?** Sí, confirmado por timestamps: `source/reimpl/io.c` se editó a las 21:18:35, `build/shadowguardian.vpk` se regeneró a las 21:19:11 (35s después, ya con el fix compilado adentro), y recién a las 21:23:08 se hizo el commit. El VPK que se le mandó al tester la noche del 11 sí contiene el fallback de Bug #24 — este no es un caso de build desactualizado.
- **Por qué falló igual:** el log muestra DOS fallos de `stat()`, no solo uno:
  ```
  stat(ux0:data/shadowguardian/gui_1_6 -> ux0:data/shadowguardian/gui_1_6): -1       (línea 999)
  stat(ux0:data/shadowguardian/sprites_1_6 -> ux0:data/shadowguardian/sprites_1_6): -1  (línea 4889)
  ```
  En ambos casos el path de salida es IGUAL al de entrada (`translate_path` devolvió el path sin resolver), lo cual solo pasa si **ninguna** de las 2 ubicaciones candidatas (`DATA_PATH<archivo>` y `DATA_PATH GloftSGHP/<archivo>`) existe físicamente en la consola del tester. La copia de referencia en este repo (`ux0_data/shadowguardian/`) SÍ tiene `gui_1_6` (24 KB), `gui_1_7`, `sprites_1_6` (66 MB) y `sprites_1_7` los 4 en la raíz — o sea, cuando la carpeta se copia completa a la Vita, esto no debería pasar. Conclusión: la consola de este tester tiene una copia incompleta de `ux0:data/shadowguardian/` a la que le faltan específicamente `gui_1_6` y `sprites_1_6` (ni planos ni bajo `GloftSGHP/`).
  - El primer fallo (`gui_1_6`, línea 999) fue tolerado por el motor (`</gui_GTab>` sin crash, sigue cargando `text/`) — evidentemente ese Lib en particular tiene un camino de fallback propio dentro del motor. El segundo (`sprites_1_6`, línea 4889) no lo tiene y es el que crashea.
  - El check de arranque de `init.c` (Bug #23) solo validaba `sprites_1_6`/`sprites_1_7`/`GloftSGHP/sprites_1_6` — nunca validó `gui_1_6`, así que una copia a la que le falta justo `gui_1_6` (o `sprites_1_6` con un `sprites_1_7` viejo satisfaciendo el check) pasa el arranque igual y crashea recién en gameplay con un native abort opaco en vez de un diálogo claro.
- **Fix aplicado:** en `source/utils/init.c`, se agregó una validación de arranque para `gui_1_6`/`gui_1_7`/`GloftSGHP/gui_1_6`/`GloftSGHP/gui_1_7` (mismo patrón que ya existía para sprites), y se sumó `GloftSGHP/sprites_1_7` a la lista de ubicaciones válidas del check de sprites que faltaba. Esto no arregla el crash en sí (la data sigue faltando), pero convierte un native abort indescifrable en un diálogo `fatal_error` explícito apenas arranca el juego.
- **Build:** `psvita-toolkit build` compila limpio, VPK regenerado.
- **Acción para el tester (no es un bug de código, es data faltante):** pedirle que verifique/vuelva a copiar `gui_1_6` (24 KB) y `sprites_1_6` (66 MB) directamente a `ux0:data/shadowguardian/` (no hace falta la subcarpeta `GloftSGHP/`, alcanza con la raíz plana). Puede confirmarlo desde VitaShell viendo el tamaño de ambos archivos en esa carpeta antes de volver a probar.
- [ ] Pendiente confirmar en consola física: redeployar este build y, si el tester ya tiene los 2 archivos completos, pedir un log nuevo — no debería verse más el crash en `_Locale_long_d_fmt`. Si a un tester futuro le sigue faltando algo, ahora debería aparecer un diálogo `fatal_error` legible en vez de un crash nativo.

### Bug #26 (confirmado 2026-09-15, sobre corrida del 2026-09-14): crash al cachear un shader que falló su compilación — mitad "escritura" del bug de shader cache del Bug #23, nunca cubierta

- **Log/dump analizados:** `game_log_1789429193443.txt` + `psp2core-1789429218-0x00007a21ed-eboot.bin.psp2dmp` (corrida 2026-09-14 ~16:40 -04, ya con el fix del Bug #25 compilado adentro — el chequeo de arranque de `gui_1_6`/`sprites_1_6` pasó sin disparar `fatal_error`, confirmado por `stat()` mtime del build vs. del fuente). El log llega hasta gameplay real (carga de shaders/modelos de armas, línea ~5053) y se corta a mitad de la carga del shader `interactible_bump`, justo después de abrir su `.vs` y antes de cerrarlo.
- **`psvita-toolkit analyze` sobre el dump:** Data abort. `PC = 0xe0002456` (`SceLibKernel seg1 + 0x16`, un stub de sistema real — `sceClibMemcpy`, detrás de `vgl_fast_memcpy`, ver `lib/vitagl/source/utils/mem_utils.h:35`). Como el `PC` cae en un módulo real de Sony, el frame que importa es `LR = 0x81038f65`, que resuelve exacto a `serialize_shader` en `lib/vitagl/source/custom_shaders.c:580` (la instrucción de retorno justo después de la llamada a `vgl_fast_memcpy(buf, s->prog, s->size)` de la línea 578). El backtrace de pila confirma un nivel más arriba: `compile_shader` en `custom_shaders.c:682` (el `sceIoWrite` justo después de la llamada a `serialize_shader` de la línea 681).
- **Causa raíz (confirmada leyendo `compile_shader()`, no por hipótesis):** el bloque `#ifdef HAVE_SHADER_CACHE` al final de `compile_shader()` (líneas 678-685 antes del fix) llamaba a `serialize_shader()` y `sceIoWrite()` **incondicionalmente**, sin comprobar si `s->prog` (el resultado de `shark_compile_shader_extended()`, un par de líneas antes) era `NULL`. Cuando el compilador Shark falla en compilar el shader (`s->prog` queda `NULL`) pero no resetea `s->size` (que puede quedar con basura de una entrada de pool reusada), `serialize_shader()` termina llamando `vgl_fast_memcpy(buf, s->prog=NULL, s->size=<basura>)` — exactamente el Data abort observado (fuente `NULL`, tamaño no-cero). El hotfix del Bug #23 (`3c9d3fb`) blindó el lado de **lectura** del shader cache (`unserialize_shader`, `glLinkProgram` cargando desde archivo) pero nunca tocó el lado de **escritura** en `compile_shader` — este es el bug hermano que quedó pendiente.
- **Fix aplicado:** en `lib/vitagl/source/custom_shaders.c`, el bloque de cacheo de `compile_shader()` ahora está envuelto en `if (s->prog) { ... }`, y se agregó chequeo de `f >= 0` tras `sceIoOpen` (mismo patrón defensivo que el resto del archivo). Un shader que falla su compilación ya no se intenta cachear — el juego sigue sin ese shader (comportamiento existente de `shark_compile_shader_extended` fallando), pero no crashea.
- **Build:** `psvita-toolkit build` compila limpio, VPK regenerado.
- [ ] Pendiente confirmar en consola física: redeployar este build y pedir al tester que repita la sesión que crasheó (carga de nivel con el shader `interactible_bump`/armas). Si el fix es correcto, no debería volver a aparecer un Data abort con `LR` en `serialize_shader`/`compile_shader`. Si la carga del nivel ahora sigue pero se ve algún material/efecto sin textura o sin sombreado correcto, eso sería la superficie visible de que ESE shader en particular sigue sin poder compilar en el compilador Shark de la Vita (haría falta el log completo con `HAVE_SHARK_LOG` para ver el mensaje de error del compilador y decidir si vale la pena simplificar ese GLSL).

### Optimización #27 (aplicada 2026-09-15, sobre corrida del 2026-09-15): baja de FPS al disparar/rodar/acciones — I/O sincrónico de SFX repetido en el hilo principal

- **Síntoma reportado:** caídas de FPS notorias al disparar, rodar, o hacer acciones distintas a simplemente caminar.
- **Log analizado:** `logs/game_log_1789505677296.txt`. No hay contadores de FPS/frame-time durante gameplay real en este log (solo el video de intro los loguea), así que el diagnóstico se hizo por patrón de I/O, no por un número de frame-time todavía.
- **Hallazgo:** el motor (`SoundMgr`, confirmado en el Bug #17 como parte de `Game::FrameUpdate()` — o sea, el hilo principal) hace un `fopen()`+`fread()`+`fclose()` completo sobre el **mismo** `.wav` corto cada vez que se dispara ese efecto de sonido — nunca cachea el audio decodificado en memoria. Caminando esto pasa ~1 vez por paso (líneas 6506-6524 del log: 3-4 variantes de `sfx_fs_walk_sand_*.wav` abriéndose por pisada). Pero al disparar/rodar/entrar en combate, se acumulan **5-8+ aperturas de archivo concurrentes** en el mismo puñado de líneas del log (líneas 6618-6763: `sfx_m16_shoot.wav`, varios `sfx_rock_bullet_impact*.wav`, varios `sfx_bullet_ricochet_generic_*.wav` simultáneos por los múltiples impactos/rebotes de cada ráfaga). Cada apertura es un viaje síncrono `stat()`+`fopen()`+`fread()`+`fclose()` a `ux0:` (tarjeta de memoria/almacenamiento interno) en el hilo crítico de frame — una causa muy plausible de los bajones de FPS específicamente en combate/rodadas vs. caminar.
- **Fix aplicado:** en `source/reimpl/io.c`, `fopen_soloader()` ahora cachea en RAM el contenido completo de cualquier archivo bajo `sounds/` (prefijo del path tal como lo pide el motor, antes de traducir), abierto en modo lectura, de hasta 1 MB — la primera vez que se lee de verdad de disco, y sirve todas las aperturas siguientes del mismo archivo desde ese buffer en memoria, sin tocar el storage de nuevo. Cada apertura (incluso concurrente, del mismo archivo) recibe su propio cursor de lectura independiente sobre el mismo buffer compartido de solo lectura — mismo comportamiento que descriptores de archivo reales independientes. Alcance deliberadamente acotado a `sounds/` (excluye `music/`, modelos, texturas, shaders, que ya tienen su propia historia de bugs de I/O documentada en este archivo).
  - Como `fread`/`fseek`/`ftell`/`feof` iban antes DIRECTO a `SceLibcBridge_*`/newlib (no pasaban por nuestro código, a diferencia de `fopen`/`fclose`), se agregaron wrappers propios (`fread_soloader`, `fseek_soloader`, `ftell_soloader`, `feof_soloader`) que reconocen un handle cacheado y lo despachan en memoria, o si no, pasan derecho a la implementación real de siempre (comportamiento 100% idéntico al actual para todo lo que no sea `sounds/*.wav`).
  - Se confirmó por `objdump -T` que el `.so` también importa `ferror`/`ungetc`/`setvbuf`/`freopen`/`fsetpos`/`fgetpos` — se agregaron wrappers equivalentes para esos también (degradan de forma segura si alguna vez se llaman sobre un handle cacheado, en vez de desreferenciar un puntero falso) aunque no se espera que la ruta de carga de `.wav` los use.
- **Build:** `psvita-toolkit build` compila limpio, VPK regenerado.
- [ ] Pendiente confirmar en consola física: redeployar y repetir una sesión con disparos/rodadas/combate. Si el fix ayuda, debería sentirse más estable el framerate en esos momentos. Sería ideal generar los hooks de `psvita-toolkit perf-telemetry --gen-hooks` en una próxima pasada para tener un número real de frame-time antes/después en vez de solo la sensación subjetiva del tester — este fix se aplicó directo (sin ese profiling previo) por ser la causa más plausible según el patrón de I/O del log, pero no está descartado que los shaders de blur al apuntar (`aim_rifle_blur`/`aim_sniper_blur`, vistos en el mismo log) también contribuyan al bajón y merezcan revisión aparte si el problema persiste tras este fix.

### Bug #28 (confirmado 2026-09-16): `interactible_basic`/`_bump`/`_shadowmap.xml` nunca compilan en el compilador Shark de la Vita — se confirma la sospecha dejada abierta en el Bug #26

- **Cómo se encontró:** comparando `ux0_data/shadowguardian` (la carpeta de datos ya en uso en la consola de pruebas) contra el dump limpio `GloftSGHP/` (extraído sin tocar del APK original). Los dos árboles son idénticos byte a byte excepto por `shaders/interactible_basic.xml`, `shaders/interactible_bump.xml` y `shaders/interactible_shadowmap.xml`, que en `ux0_data` tienen ediciones manuales que no existen en ningún lado del repo ni de los commits: `vec4(1)` → `vec4(1.0)` en los tres, y en `_bump` además se agregó la declaración `uniform highp float OutlineSize;` que faltaba.
- **Causa raíz:** el compilador Shark de vitaGL (el que reemplaza al driver GLES de Android en tiempo de ejecución) es más estricto que el driver de Android con estos 3 shaders del motor: no acepta el constructor `vec4(1)` sin literal flotante explícito, y en `_bump` además falla por referenciar `OutlineSize` sin haberlo declarado como `uniform`. El resultado es exactamente el fallo de compilación (`s->prog` queda `NULL` en `compile_shader()`) que ya se había mitigado del lado del crash en los Bugs #23/#26 — esos fixes evitan el Data abort, pero nunca hicieron que el shader compilara, así que el tester seguía sin ver ese material/efecto renderizado correctamente (justo el escenario que el Bug #26 dejó anotado como pendiente de revisar).
- **Por qué el tester no podía jugar bien pese a los fixes de crash:** los 3 shaders afectados son de objetos "interactuables" (`interactible_*` — ítems/objetos con los que el jugador interactúa en el nivel), así que el efecto visible no es un crash sino geometría sin sombrear o sin su material correcto en esas zonas del juego.
- **Fix aplicado:** los 3 archivos ya corregidos manualmente en `ux0_data/shadowguardian/shaders/` se copiaron al repo en `extras/shader_patches/` y se agregaron al VPK (`CMakeLists.txt`, `vita_create_vpk`) en la ruta interna `shader_patches/*.xml`. Se agregó `apply_shader_patches()` en `source/utils/init.c`, llamada desde `soloader_init_all()` justo después del chequeo de arranque de la carpeta `res/` (antes de cargar el `.so`): copia los 3 shaders corregidos desde `app0:shader_patches/` hacia `shaders/` dentro de `DATA_PATH`, detectando si el tester instaló los datos en layout plano o bajo `GloftSGHP/`, y sobreescribe sin condición en cada arranque (operación barata — 3 archivos de texto de pocos KB — e idempotente, así que no importa si el tester ya tenía los shaders parcheados o los originales del APK).
- **Build:** pendiente compilar con `psvita-toolkit build` (este entorno no tiene vitasdk) y regenerar el VPK.
- [ ] Pendiente confirmar en consola física: redeployar en una consola SIN el parche manual previo en `ux0_data/shadowguardian/shaders/` (o con los shaders originales del APK) y confirmar que los objetos interactuables ahora se ven sombreados/texturizados correctamente, sin regresar el crash de los Bugs #23/#26.

### Mejora #29 (aplicada, sin confirmar en consola, 2026-09-19): ocultar también el selector de arma en pantalla — R/L y TRIANGLE/CROSS ya cubren disparo/apuntado/cambio de arma/agarre físicamente

- **Pedido:** el usuario confirmó que R/L deben seguir siendo disparar/apuntar (ya así en `source/main.c`: R = FIRE, L o SQUARE = AIM), que los botones de acción física (TRIANGLE = cambio de arma, CROSS = acción contextual que incluye agarrar objetos) ya cubren esas dos funciones, y pidió ocultar el botón de arma en pantalla (antes dejado visible a propósito, ver Bug #10/Intento 2026-09-11) y confirmar que el botón de mira ("el que está activo en la parte inferior") también quede oculto.
- **Mapeo físico:** sin cambios en `source/main.c` — ya cumple lo pedido (R=disparo, L/SQUARE=apuntar, TRIANGLE=cambio de arma, CROSS=agarrar/interactuar contextual).
- **Cambio aplicado (`source/patch.c`, `set_virtual_buttons_visible`):**
  - Se eliminó la exclusión deliberada del `ButtonEnum 7` (selector de arma) que antes se dejaba SIEMPRE visible a pedido explícito de una sesión anterior.
  - Se agregó un segundo barrido incondicional de alpha sobre los índices `0xb..0x20` (11-32 decimal) — el rango EXACTO que el propio `GS_GamePlay::SetButtonVisible` usa para ocultar el `case 7` (confirmado en `decompiled/libshadowguardian_armeabi-v7a/ghidra/out_ghidra.c:160029-160049`, no adivinado), en vez de los 3 índices parciales (`0xb, 0x1c, 0x1d`) mencionados en el fix anterior. Usa la misma `set_item_alpha_safe` (bound-check contra `m_elementsCount`) que ya blinda el resto de esta función contra el crash de los Bugs #19/#19b.
  - El barrido de índices `0..10` (joystick + botones de acción + tutorial, incluye el ítem 7 compartido que corresponde al botón de mira) queda sin cambios — ya estaba diseñado para cubrir el botón de apuntar (ver Intento 2026-09-11), así que si sigue "activo" en consola tras este build, el siguiente sospechoso NO es el rango de índices sino que el toggle (`g_hide_virtual_buttons`, CIRCULO) esté en estado visible al momento de probar, o que `set_virtual_buttons_visible` no se haya podido aplicar aún (GUIMgr/GS_GamePlay no vivos todavía).
- **Build:** pendiente compilar con `psvita-toolkit build` (este entorno no tiene vitasdk) y regenerar el VPK.
- [ ] Pendiente confirmar en consola física: redeployar, entrar a gameplay real y verificar que NI el selector de arma NI el botón de mira queden visibles en la esquina inferior, y que disparar (R), apuntar (L), cambiar de arma (TRIANGLE) y agarrar/interactuar (CROSS) sigan funcionando igual que antes (el ocultamiento es solo de opacidad, no desactiva el área táctil real).

### Mejora #30 (aplicada, sin confirmar en consola, 2026-09-19): triage con log real (`game_log_1789869263549.txt`) de la Mejora #29 — coordenadas de botones mal medidas, ocultamiento con "latch" que se revertía solo, glow de highlight independiente del alpha, e intro borrosa por downsampling

- **Reporte del usuario tras probar la Mejora #29 en consola:** TRIANGLE dispara (en vez de cambiar de arma), CIRCULO vuelve a mostrar los controles táctiles (no se quería más ese toggle), CUADRADO sigue apuntando (debía ser solo L), el botón de mira sigue visible al ocultar controles, y aparece "un efecto azul sobre un botón invisible" cuando el juego pide tocar un botón. Aparte, pidió mejorar nitidez/escalado del video de intro.
- **Causa raíz 1 (TRIANGLE dispara) — coordenadas de `F_WPN_X/Y` nunca confirmadas, y estaban cerca del hitbox de FIRE:** el comentario original en `source/main.c` ya marcaba esa posición como "GUESS". Re-medí los 5 botones reales sobre `screenshots/dc/2026-09-06/2026-09-06-033741.jpg` (960x544) con una grilla de píxeles superpuesta (`PIL`, ver metodología abajo) en vez de estimar a ojo:
  - FIRE (icono de pistola, esquina inferior derecha): real (750, 511) → antes se usaba engine (621,422), ahora (625,451) — ya estaba razonablemente bien.
  - AIM (mira/retícula, esquina inferior derecha): real (905, 503) → engine (754, 444), antes (738,432) — bien, ligero ajuste.
  - ACT (icono de "hombre corriendo", columna derecha medio): real (870, 374) → engine (725, 330), antes (738,304) — bien, ligero ajuste.
  - GRAB (icono de "mano", columna derecha arriba): real (870, 172) → engine (725, 152) — **no tenía físico asignado antes** (ver Causa raíz 2).
  - ARMA (icono de pistola equipada, arriba a la derecha, junto al contador de balas): real (825, 34) → engine (688, 30). El código anterior usaba engine (542, 424), que convertido de vuelta a pantalla real es (650, 480) — a ~105px del centro real de FIRE (750,511), dentro del radio táctil típico de un botón táctil grande estilo Gameloft (~100px de radio medido en la propia captura). Eso explica exactamente por qué TRIANGLE (atado a esa posición) disparaba en vez de cambiar de arma.
  - Metodología: `python3` + `Pillow`, recortando la región de cada botón y dibujando una grilla etiquetada cada 20px sobre una versión 2x/3x del crop (script ad-hoc, no se guardó en el repo por ser un scratch de medición), para leer el centro de cada círculo con precisión de pocos píxeles en vez de estimarlo de la imagen completa.
- **Causa raíz 2 (falta un botón para "agarrar objetos" + CIRCULO hacía toggle no deseado):** el ícono de "mano" (interactuar/agarrar) nunca tuvo un botón físico asignado — CRUZ estaba atado al ícono de "correr", no al de la mano. CIRCULO solo tenía el toggle de depuración `g_hide_virtual_buttons` (mostrar/ocultar), que el usuario ya no quiere.
- **Causa raíz 3 (SQUARE seguía apuntando):** `source/main.c` compartía el toque de AIM entre `SCE_CTRL_LTRIGGER | SCE_CTRL_SQUARE` a propósito (línea vieja: "Aim: L trigger (hold). Shares the spot with SQUARE below.") — el usuario pidió explícitamente que solo L apunte.
- **Causa raíz 4 (botón de mira "sigue visible" pese al fix de la Mejora #29):** el ocultamiento (`set_virtual_buttons_visible`) se aplicaba UNA sola vez (`virtual_buttons_state_synced` quedaba `true` para siempre tras el primer éxito) apenas arrancaba la gameplay. Si el motor vuelve a mostrar ese ítem por su cuenta más tarde (feedback contextual al apuntar, hints de tutorial, etc.), nuestro forzado de alpha=0 nunca se reaplicaba — quedaba "pegado" visible.
- **Causa raíz 5 (glow azul sobre botón invisible):** `GS_GamePlay::HighlightButton(ButtonEnum, int)` (`out_ghidra.c:158911`) es una función completamente separada de `GUILevel::SetItemAlpha` — solo escribe `*(int*)(this + (param_1+0xe)*4) = param_2`, un timer/intensidad de resaltado por botón que otra rutina de dibujo lee para pintar un glow/pulso ENCIMA del botón, en un pase de render independiente del alpha del ítem. El motor la llama solo (feedback de apuntado, hints de "toca este botón"), así que el glow se veía igual aunque el botón de abajo tuviera alpha 0 — esto explica tanto el "efecto azul" como parte de la sensación de "botón de mira que sigue visible".
- **Causa raíz 6 (video de intro borroso):** `source/video.cpp` tenía `VIDEO_DOWNSAMPLE_UPLOAD` activado — reducía el frame decodificado de 480x272 a 240x136 antes de subirlo a textura, y el viewport (correcto en aspect ratio) lo estiraba 4x de vuelta a 960x544 con `GL_LINEAR`, produciendo un blur muy visible. La reducción se había agregado para ahorrar ancho de banda de subida, pero el intro dura ~3s y se reproduce una sola vez.
- **Fix aplicado:**
  1. `source/main.c`: coordenadas `F_FIRE_X/Y`, `F_AIM_X/Y`, `F_ACT_X/Y`, `F_WPN_X/Y` actualizadas a las remedidas; nueva `F_GRAB_X/Y` (ícono de mano). AIM ahora solo con `SCE_CTRL_LTRIGGER` (se quitó `SCE_CTRL_SQUARE`). CIRCULO ya no hace toggle de visibilidad — ahora dispara GRAB (`FID_GRAB`) igual que R/L/CRUZ/TRIÁNGULO hacen sus acciones. El bucle principal llama `set_virtual_buttons_visible(!g_hide_virtual_buttons)` **todos los frames** (antes solo hasta el primer éxito) para que cualquier re-aparición del motor se revierta casi al instante en vez de quedar pegada.
  2. `source/patch.c`: se agregó el hook `GS_GamePlay_HighlightButton_hook`, registrado sobre `_ZN11GS_GamePlay15HighlightButtonENS_10ButtonEnumEi`. Como se conoce el cuerpo completo de la función original (una sola escritura de memoria), se reimplementa directo en vez de usar un trampolín: si `g_hide_virtual_buttons` es `true`, siempre escribe `0` (sin resaltado) en lugar del valor pedido por el motor.
  3. `source/video.cpp`: `VIDEO_DOWNSAMPLE_UPLOAD` pasado de `1` a `0` — el intro ahora sube el frame decodificado a resolución completa (480x272, exactamente 2x hacia 960x544 con `GL_LINEAR`, aspect ya correcto de antes).
- **Build:** `psvita-toolkit build` compila limpio, VPK regenerado (`build/shadowguardian.vpk`).
- [ ] Pendiente confirmar en consola física (mandar log nuevo): (a) TRIANGLE cambia de arma sin disparar; (b) CIRCULO agarra/interactúa (icono de mano) y ya NO muestra los controles táctiles; (c) CUADRADO no hace nada (solo L apunta); (d) ni el botón de mira ni ningún otro control táctil quedan visibles durante gameplay, incluyendo momentos de apuntado/QTE/tutorial (sin el glow azul); (e) el video de intro se ve nítido. Si el glow azul persiste pese al hook de `HighlightButton`, el siguiente sospechoso es que exista una SEGUNDA función de highlight no descubierta aún (buscar otras referencias a offsets `this+0xe*4` en el pseudo-C) en vez de volver a tocar `set_virtual_buttons_visible`.

### Mejora #31 (aplicada, sin confirmar en consola, 2026-09-20): triage con log real (`game_log_1789877027578.txt`) de la Mejora #30 — video confirmado OK; cambio de arma, toggle de controles y cámara del stick derecho corregidos

- **Confirmación del usuario:** el video de intro "está mejor" (Mejora #30, punto (e), confirmado).
- **Nuevos pedidos/bugs sobre la Mejora #30:**
  1. El cambio de arma no funciona como debería: el usuario aclaró que el control real NO es un tap simple sobre el ícono del arma, sino "un botón pequeño tipo slider/flechas a los lados del arma" para seleccionar otra, y que ese control debe estar en CUADRADO.
  2. Pidió que SELECT haga aparecer los controles táctiles, como hacía CIRCULO antes de la Mejora #30 (CIRCULO ahora es GRAB, ver Mejora #30).
  3. El stick derecho (cámara) gira rápido pero está "limitado a un área" — no permite ver un 360 lento y preciso del personaje.
  4. Pidió ocultar también "el efecto azul de seleccionar el arma".
- **Investigación (sin adivinar a ciegas esta vez, tras el error de coordenadas de la Mejora #30):**
  - Encontré `out_ghidra.c:86760-86813` (el manejador de input de `PlayerCtrl`, en la función que también llama `UpdateFreeCamera()`/`UpdateInputEvents()`/`UpdateControls()`): la lógica real del selector de arma NO es "tap en X,Y = cambiar arma". Ubica el toque activo con `TouchManager::FindTouch(this,3,4)` y calcula el DELTA de arrastre (`curX - startX`) de ESE toque. Si el arrastre supera ~30 unidades, el SIGNO solo alcanza para decidir `Actor::SetNextWeapon()` (arrastrado a la derecha) o `Actor::SetPreviousWeapon()` (arrastrado a la izquierda) — coincide exactamente con la descripción del usuario ("slider tipo flechas"). Solo el caso de un tap casi sin arrastre cae a comparar contra umbrales fijos (365/450) cuyo sistema de coordenadas no pude confirmar (no coinciden con la posición real del ícono en pantalla, así que probablemente sean de un espacio de referencia interno, no los mismos píxeles de motor 800x480 que usa `nativeOnTouch`).
  - Decisión: en vez de arriesgar otra posición mal adivinada (como pasó con `F_WPN` en la Mejora #29/#30), CUADRADO ahora sintetiza un GESTO DE ARRASTRE completo (`DOWN` en el ícono del arma ya confirmado → `MOVE` 100px a la derecha → `UP`) en vez de un tap. La lógica de "arrastre grande" solo depende del SIGNO del delta, no de los umbrales 365/450 en duda, así que funciona sin importar en qué espacio de coordenadas estén esos dos números.
  - Confirmé con `nm -D` sobre el `.so` original que `Actor::SetNextWeapon()`/`Actor::SetPreviousWeapon()` existen como funciones reales del motor (no alucinadas): `000e15d4 T _ZN5Actor13SetNextWeaponEv`.
- **Causa raíz (cámara limitada):** `source/main.c` mapeaba el stick derecho a una POSICIÓN absoluta (`cam_x = F_CAM_X + dz_rescale(rx,250)`) en vez de una velocidad relativa — al llegar al tope de 250px el toque sintético simplemente se quedaba quieto ahí (sin importar cuánto siguiera sostenido el stick), y como el gesto es un "arrastre" (el motor rota la cámara por el DELTA de movimiento del toque, no por su posición absoluta), quedarse quieto en el tope significa cero rotación adicional — de ahí "rápido pero limitado a un área", sin forma de dar una vuelta completa.
- **Fix aplicado (`source/main.c`):**
  1. **Arma:** `SCE_CTRL_TRIANGLE` → `SCE_CTRL_SQUARE` para el cambio de arma, y la acción pasó de un tap a un gesto `DOWN`→`MOVE(+100px)`→`UP` (`F_WPN_SWIPE_DX`) sobre la posición ya confirmada del ícono.
  2. **SELECT:** ahora alterna `g_hide_virtual_buttons` (mostrar/ocultar controles táctiles), reemplazando el envío de `KEYCODE_MENU` que tenía antes (no se vio usado por ningún flujo real hasta ahora).
  3. **Cámara:** rediseñada como arrastre CONTINUO relativo en vez de posición absoluta. El stick ahora setea una VELOCIDAD por frame (`F_CAM_SPEED=10`, escalada por inclinación vía `dz_rescale` — inclinación suave = giro lento y preciso, inclinación a fondo = giro rápido) que se acumula en `cam_x/cam_y` mientras se sostiene el stick. Al acumular más de `F_CAM_RADIUS=200` px de distancia del ancla, se suelta el toque en el borde y se vuelve a presionar en el centro EN EL MISMO FRAME (invisible para el jugador) para poder seguir arrastrando en la misma dirección indefinidamente — permite ahora sí una vuelta de 360° completa, lenta y precisa si se inclina poco, o rápida si se inclina a fondo.
- **Sin resolver todavía:** el "efecto azul de seleccionar el arma". El hook genérico de `GS_GamePlay::HighlightButton` (Mejora #30) sí cubre el `ButtonEnum` 7 (arma) según el wrapper Lua en `out_ghidra.c:192747`, pero no tengo evidencia de log de que sea la MISMA fuente del glow que ve el usuario al cambiar de arma — podría ser un efecto de partículas/flash distinto dentro de `Actor::ChangeWeapon`/`PlayerCtrl::UpdateWeaponGUI` que no pude ubicar por análisis estático. No se tocó nada más para esto por ahora (evitar otro hook a ciegas que podría crashear) — si sigue apareciendo tras confirmar el resto, el siguiente paso es aislarlo con un log/captura del momento exacto del cambio de arma.
- **Build:** `psvita-toolkit build` compila limpio, VPK regenerado (`build/shadowguardian.vpk`).
